...
def result_json():
    ...
    status_val = -1
    dic={-1:"請選取日期或地點",0:"準時抵達",1:"delay 0 ~ 30 分鐘",2:"delay 30 ~ 60 分鐘",3:"delay 60 ~ 90 分鐘",
        4:"delay 90 ~ 120 分鐘",5:"delay 120 ~ 150 分鐘",6:"delay 150 分鐘以上"}

    if len(date) == 0 or len(startairport) == 0 or len(endairport) == 0:
        return render_template("2Dashboard.html", answer=dic[status_val], CCcolor="#D7A1FF")
    else:
        Year = date.split('-')[0]
        Month = date.split('-')[1]
        DayofMonth = date.split('-')[2]

        place = pd.read_excel('./機場對照.xlsx')
        ...
        fliter = (place["NAME"] == startairport)
        fliter = place[fliter]
        startairport = str(fliter.iat[0,0])
        Dest_dict = np.load('Dest_map.npy', allow_pickle='TRUE')
        startairport = Dest_dict.item()[startairport]

        fliter = (place["NAME"] == endairport)
        fliter = place[fliter]
        endairport = str(fliter.iat[0,0])
        Origin_dict = np.load('Origin_map.npy', allow_pickle='TRUE')
        endairport = Origin_dict.item()[endairport]

        input_list = [Year, Quarter, Month, DayofMonth, DayOfWeek,
        IATA_Code_Marketing_Airline, startairport, endairport, CRSDepTime,
    ...
    colorDic={-1:"#D7A1FF",0:"#ABFFAB",1:"#FFE1AB",2:"#FFE1AB",3:"#FFE1AB",4:"#FFE1AB",5:"#FFABAB",6:"#FFABAB"}   

    e1=endairport
    filt =(x['DATE']==date)&(x['NAME']==e1)
    y=x[filt]['AWND'].values
    return render_template("2Dashboard.html", startcontinent=startcontinent,startairport=Origin,
    endcontinent=endcontinent,endairport=Dest,date=date,y=y,AWND_x=AWND_x,PRCP_x=PRCP_x,TMAX_x=TMAX_x,
